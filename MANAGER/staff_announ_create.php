...
<!-- <input type="checkbox" id="check"> -->
    ...
    <form action="staff_announ_create.php" method="POST">

            <label for="staf_announ_date">Title*</label>
            <input type="date" name="staf_announ_date" required><br><br>

            <label for="staf_announ_title">Title*</label>
            <input type="text" name="staf_announ_title" required><br><br>

            <label for="staf_announ_desc">Description*</label>
            <input type="text" name="staf_announ_desc" required><br><br>

            <div class="center-button"> <!-- Added div for centering -->
                <input type="submit" name="staf_announ_save" value="Save">
            </div>
...
<?php
if(isset($_POST['staf_announ_save'])){

    // current date
    
    // storing form details
    $date = $_POST['staf_announ_date'];
    $title = $_POST['staf_announ_title'];
    $desc = $_POST['staf_announ_desc'];
    $aud = "staff";
    ...
    $stmt = $conn->prepare("INSERT INTO announcement (announcement_date, title, description, audience) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $date, $title, $desc, $aud);

    if ($stmt->execute()) {
        echo "<script>alert('Announciment Created Successfully...!');
        window.location.href = 'view_staff_announ.php';
        </script>";
    } else {
        echo "<script>alert('some error occured...!<br>.$conn->error');
        window.location.href = 'staff_announ.php';
        </script>";
    }
